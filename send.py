...
def zipfile(dir, dest):
    subprocess.run("7z a -o{" + dir + "} -t7z " + dir+"/"+dest + ".7z " + dir+"/"+dest + ".dat -m9=LZMA2 -aoa", shell=True)
...
while True:
    ...
    if msg_type == 0xDF:
        ...
        cur_day = (int(cur_tow) + 2) // 60 // 60 // 24 % 7
        if cur_day != day:
            multiprocessing.Process(target=zipfile, args=(copy.deepcopy(home_path+str(wn)), copy.deepcopy(str(wn)+"_"+str(day)))).start()
            day = cur_day
            wn = cur_wn
            try:
                os.mkdir(home_path + str(wn))
